...
**Your backend is running on:** `localhost:8404` ✅
...
**Change the port from `8585` to `8404`:**
...
// AFTER (Correct):
const javaURI = "http://localhost:8404";
```
...
export const javaURI = "http://localhost:8404";  // ← Make sure it's 8404!
...
Edit `assets/js/api/config.js` and change `8585` → `8404`
...
```bash
curl http://localhost:8404/api/post/all
# Should return: []
```
...
| Service | Port | Status |
|---------|------|--------|
| **Flask Backend** | **8404** | ✅ Correct - Use this! |
...
1. **Backend is on 8404** - This is confirmed and working ✅
2. **Don't change backend** - It's correct!
3. **Change frontend config** - Update to port 8404
...
Once you update the port to `8404`:
...
```javascript
// Backend API Configuration
export const javaURI = "http://localhost:8404";
...
# Find and replace 8585 with 8404
find assets -name "*.js" -type f -exec sed -i '' 's/8585/8404/g' {} +
...
| What | Where | Change |
|------|-------|--------|
| **Backend** | Flask (running) | Port **8404** ✅ |
| **Frontend Config** | `config.js` | Change to **8404** ❌→✅ |
...
Just update that one line in your frontend config from `8585` to `8404` and everything will work!
